...
class CONV(object):
    ...
    def conv(self, x, max_sen_len, max_doc_len, sen_len, doc_len, usr, prd):
        ...
        # conv should be tensor with shape
        # [batch_size, d, max_doc_len-4, 1]
        # here a real batch is considered as max_doc_len batches
        caps_sen = tf.reshape(caps_sen, [-1, max_doc_len, self.aspect_cnt * d, 1], name='caps_sen_for_conv')
        conv = tf.layers.conv2d(caps_sen, d, [5, self.aspect_cnt * d])
        # [batch_size, max_doc_len-2, d, 1]
        ...
        caps_doc, _ = cl.layers.dense(inputs=conv, activation=activation,
                                      num_outputs=self.aspect_cnt, out_caps_dims=[d, 1],
                                      sequence_length=doc_len - 4,
                                      share=True,
                                      max_length=max_doc_len - 4,
    ...
    def build(self, data_iter):
        ...
        d_hat = self.conv(input_x, self.max_sen_len, self.max_doc_len,
